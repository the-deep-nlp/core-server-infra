...
from prompts import (analytical_statement_prompt, information_gaps_prompt,
...
class LLMSummarization(SummarizerBase):
    ...
    def summarizer(self, tags: list, min_tokens: int = 20):
        ...
        with get_openai_callback() as cb:
            generated_summary = summarize_chain.invoke(
                {"text": self.texts, "tags": tags, "tokens_count": tokens_count}
            )
            summary_info = {
    ...
    def generate_analytical_statement(self, summary: str):
        ...
        template_prompt = PromptTemplate.from_template(prompt)
        chain = LLMChain(llm=self.llm, prompt=template_prompt)

        with get_openai_callback() as cb:
            generated_analytical_statement = chain.invoke({"summary": summary})
        ...
        return (
            (
                generated_analytical_statement["text"]
                if "text" in generated_analytical_statement
                else ""
            ),
            analytical_statement_info,
        )
    ...
    def generate_information_gaps(self, entries: list, topics: list):
        ...
        with get_openai_callback() as cb:
            generated_info_gaps = chain.invoke({"entries": entries, "topics": topics})
